  ...
  <div class="page-wrapper">
    ...
    <main v-if="asset" class="detail-layout">
      <div v-if="isDeprecated" class="deprecated-banner">
        <div><strong>지원 종료된 스킬입니다.</strong><p>최신 환경에서는 정상적으로 동작하지 않을 수 있습니다.</p></div>
        <router-link v-if="replacementAssetId" :to="{ name: 'CourseDetail', params: { id: replacementAssetId } }">대체 스킬 보기 →</router-link>
      </div>
        ...
        <div class="detail-hero-inner">
          <div class="detail-info fade-in-up">
            ...
            <div class="detail-meta">
              <span>등록자: {{ displayAuthorName }}</span>
      ...
      <section class="install-section">
        ...
        <div class="install-card">
          ...
          <template v-else-if="installState === 'REQUESTABLE'">
            <span class="lock">🔐</span><h3>승인이 필요한 스킬입니다</h3>
      ...
      <section v-if="relatedAssets.length" class="related-section">
        <div class="section-heading"><div><small>RELATED</small><h2>같은 분야의 인기 스킬</h2></div></div>
    ...
    <div v-else class="loading-center empty-state"><p>스킬 정보를 불러오지 못했습니다.</p><router-link to="/courses" class="btn btn-outline">스킬 목록으로</router-link></div>
  </div>
</template>
...
const displayAuthorName = computed(() => asset.value?.instructorName || asset.value?.authorName || asset.value?.ownerName || asset.value?.instructor?.name || '등록자 정보 없음')
...
async function copyInstallCommand() {
  ...
  try {
    ...
    if (grade.value === 'PUBLIC' && enrollmentStatus.value === 'NONE') {
      ...
      }).catch(error => console.warn('[CourseDetail] 공개 스킬 사용 기록 생성 실패:', error))
    }
...
watch(enrollmentStatus, async (status, previousStatus) => {
  ...
  // 제한 스킬은 승인 전 상세 응답에 installCommand가 없어야 한다.
